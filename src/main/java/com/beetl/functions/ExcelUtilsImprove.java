 ...
 * 1.读取Excel操作基于注解映射,handler为{@link ExcelUtilsImprove#readExcel2ObjectsHandler}<br>
 * 2.读取Excel操作无映射,handler为{@link ExcelUtilsImprove#readExcel2ObjectsHandler}<br>
 * 3.基于模板、注解导出excel,handler为{@link ExcelUtilsImprove#exportExcelByModuleHandler}<br>
 * 4.基于模板、注解导出Map数据,handler为{@link ExcelUtilsImprove#exportExcelByModuleHandler}<br>
 * 5.无模板基于注解导出,handler为{@link ExcelUtilsImprove#exportExcelNoModuleHandler}<br>
 * 6.无模板无注解导出,handler为{@link ExcelUtilsImprove#exportExcelNoModuleHandler}<br>
 * <p>
...
public class ExcelUtilsImprove {
     ...
     * 单例模式
     * <p>
     * 通过{@link ExcelUtilsImprove#getInstance()}获取对象实例
     */
    static private ExcelUtilsImprove ExcelUtilsHc = new ExcelUtilsImprove();

    private ExcelUtilsImprove() {
    }

    public static ExcelUtilsImprove getInstance() {
        return ExcelUtilsHc;
    }
    ...
    private List<List<String>> readExcel2ObjectsHandler(Workbook workbook, int offsetLine,
    	...
    	short firstCellNum = row_first.getFirstCellNum(); // 该行的第一个单元格
        short lastCellNum = row_first.getLastCellNum(); // 该行的最后一个单元格
        ////////////////////////////////////////////////////////
